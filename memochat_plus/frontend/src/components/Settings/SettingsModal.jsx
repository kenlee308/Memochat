...
import { X, Download, HardDrive, Brain, Zap, Target, RefreshCw, ShieldCheck, Microscope } from 'lucide-react';
...
const SettingsModal = ({
    ...
    const handleExport = async () => {
        try {
            ...
            // Fix: handle JSON objects vs string content
            const exportContent = typeof data === 'object' ? JSON.stringify(data, null, 2) : (data.content || data);
            const blob = new Blob([exportContent], { type: exportFormat === 'json' ? 'application/json' : 'text/markdown' });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `memochat_identity_${new Date().toISOString().split('T')[0]}.${exportFormat === 'txt' ? 'md' : 'json'}`;
            a.click();
        } catch (error) {
                ...
                <div className="p-6 space-y-8 overflow-y-auto scrollbar-custom">
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-[9px] text-slate-500 leading-relaxed italic border-l-2 border-white/5 pl-3">
                            Determines the randomness of token selection. Lowering this ensures the AI adheres strictly to facts and logical constraints in your Long-Term Knowledge.
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        />
                        <p className="text-[9px] text-slate-500 leading-relaxed italic border-l-2 border-white/5 pl-3">
                            How often the AI audits its internal logic. Lower values create more surgical, accurate memories.
                        </p>
                    </div>

                    {/* 5. Neural Validation (v3.1) */}
                    <div className="space-y-6 pt-4 border-t border-white/10">
                        <label className="text-[10px] font-black uppercase tracking-widest text-indigo-400 flex items-center gap-2">
                            <ShieldCheck size={14} /> Neural Validation (v3.1)
                        </label>

                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-xs font-bold text-slate-200">Self-Correction (Similarity)</span>
                                    <span className="text-[9px] text-slate-500">Enable automated duplicate detection</span>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" checked={enableSimilarityCheck} onChange={(e) => setEnableSimilarityCheck(e.target.checked)} className="sr-only peer" />
                                ...
                                <div className="space-y-3 pl-4 border-l border-white/5">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
                                            <Microscope size={12} /> Duplicate Sensitivity
                                        </label>
                                        <span className="text-[10px] font-mono text-indigo-400 bg-indigo-400/10 px-2 py-0.5 rounded">{Math.round(similarityThreshold * 100)}%</span>
                                    </div>
                                    <input
                                        type="range"
                                        min="0.5"
                                    ...
                                    <div className="flex justify-between text-[8px] font-black uppercase tracking-tighter opacity-50">
                                        <span>⊕ Broad / Merging</span>
                                        <span>Strict / Surgical ⊖</span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-xs font-bold text-slate-200">Context-Aware Merge</span>
                                    <span className="text-[9px] text-slate-500">Show AI relevant facts during sleep</span>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" checked={enableContextAware} onChange={(e) => setEnableContextAware(e.target.checked)} className="sr-only peer" />
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex flex-col gap-0.5">
                                    <span className="text-xs font-bold text-slate-200">Auto-Resolve Conflicts</span>
                                    <span className="text-[9px] text-slate-500">Run consistency audit after consolidation</span>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" checked={autoResolveConflicts} onChange={(e) => setAutoResolveConflicts(e.target.checked)} className="sr-only peer" />
                                    <div className="w-9 h-5 bg-slate-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-purple-600"></div>
                                </label>
                            </div>

                            <p className="text-[9px] text-slate-500 leading-relaxed italic border-l-2 border-white/5 pl-3">
                                Ensuring the AI sees existing related knowledge before updating, preventing fragmented duplicates.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-white/10">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                <option value="txt" style={{ backgroundColor: '#0f172a', color: 'white' }}>Markdown (.md)</option>
                                <option value="json" style={{ backgroundColor: '#0f172a', color: 'white' }}>Schema (.json)</option>
                            </select>
                            <button
